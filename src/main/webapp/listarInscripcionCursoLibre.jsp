    ...
    <table border="1">
        ...
        <c:forEach var="i" items="${listaInscripciones}">
            ...
            <tr>
                ...
                <td>
                    <c:set var="estudianteEncontrado" value="false" />
                    <c:forEach var="e" items="${listaEstudiantes}">
                        <c:if test="${e.id == i.idEstudiante}">
                            ${e.nombre} ${e.apellido}
                            <c:set var="estudianteEncontrado" value="true" />
                        </c:if>
                    </c:forEach>
                    <c:if test="${!estudianteEncontrado}">
                        <span style="color:red;">No encontrado</span>
                    </c:if>
                </td>
                
                <td>
                    <c:set var="cursoEncontrado" value="false" />
                    <c:forEach var="c" items="${listaCursosLibres}">
                        <c:if test="${c.id == i.idCursoLibre}">
                            ${c.nombre}
                            <c:set var="cursoEncontrado" value="true" />
                        </c:if>
                    </c:forEach>
                    <c:if test="${!cursoEncontrado}">
                        <span style="color:red;">Curso no encontrado</span>
                    </c:if>
                </td>
